...
merged_soilChemistry |>
  ggplot(aes(x = organicCPercent, y = organicd13C)) +
  geom_point(aes(fill = plotID, shape = plotID), size = 3) +
  scale_shape_manual(values=c(21, 22, 23, 24, 25, 21, 22, 23, 24, 25)) +
  scale_fill_brewer(palette="Paired") +
  geom_hline(yintercept = -19, color = "red") +  # Add red horizontal line
  geom_vline(xintercept = 1, color = "blue") +  # Add blue vertical line
  annotate("text", x = 0.1, y = -16.5, label = "grass signal", 
           color = "purple", hjust = 0, vjust = 1) + 
  annotate("text", x = 2.5, y = -21, label = "shrub signal", 
           color = "purple", hjust = 1, vjust = 0) + 
  theme_minimal() + 
  theme(
...
## Coordinates for DeepForest

```{r}
#| label: initial-DeepForest-exploration

# pull out coordinates for plots 05 and 06 to look at veg context of cores 
CoreCoordinates <- merged_soilChemistry |>
    filter(plotID == "SRER_005" |plotID == "SRER_006" ) |>
    select(plotID, utmZone, adjNorthing, adjEasting, adjCoordinateUncertainty, 
           adjDecimalLatitude, adjDecimalLongitude, adjElevation, adjElevationUncertainty) |>
    distinct(adjNorthing, .keep_all = TRUE)
write.csv(CoreCoordinates, here("output", "SRER_core_coordinates.csv"), row.names = FALSE)
...
#### NOT WORKING
#| label: get-plot-coordinates

# Download site layout data for all sites
site_info <- loadByProduct(dpID = "DP1.00001.001")

# Assuming 'site_info' contains the plot data, filter for the SRER site and specific plots
srer_plots <- site_info$dp1.00001.001$site_layout %>%
  filter(siteID %in% c("SRER"), plotID %in% c("SRER_005", "SRER_006"))

# Convert coordinates from NEON's spatial reference to UTM and decimal degrees
# This step assumes the coordinates are provided in a suitable format for conversion
srer_plots_utm <- st_transform(srer_plots, crs = st_crs(32612)) # Example UTM zone 12N
srer_plots_dd <- st_transform(srer_plots, crs = st_crs(4326)) # WGS84 Decimal Degrees

# Extract the coordinate data to a new dataframe for CSV output
coordinates_df <- data.frame(
  plotID = srer_plots$plotID,
  utm_easting = st_coordinates(srer_plots_utm)[,1],
  utm_northing = st_coordinates(srer_plots_utm)[,2],
  decimal_degrees_lon = st_coordinates(srer_plots_dd)[,1],
  decimal_degrees_lat = st_coordinates(srer_plots_dd)[,2]
)

# Write the dataframe to a CSV file
write.csv(coordinates_df, here("output", "srer_plot_coordinates.csv"), row.names = FALSE)
